---
name: vendored-slack-search
description: Guidance for effectively searching Slack to find messages, files, channels, and people. Use whenever you need to find information in Slack — locate messages, conversations, files, or people, or gather Slack context before answering. Vendored copy of the upstream `slack:slack-search` skill — comp-voice's compose-mp pipeline calls this in step 3b. If the upstream `/slack:slack-search` is also installed, prefer that one. Requires the slack MCP tools (`slack_search_public`, etc.) to be connected.
---

<!-- VENDORED COPY — body verbatim from upstream.
     Source: claude-plugins-official/slack 1.0.0, skills/slack-search/SKILL.md

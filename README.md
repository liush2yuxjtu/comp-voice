...
Live site: **https://liushiyumath.github.io/comp-voice/**

A polished editorial-style landing page ships under `docs/index.html`, generated by [pitchkit](https://github.com/EricSun0218/pitchkit) `landing-page` from `docs/ctx.json` and served by GitHub Pages off the `/docs` folder on `main`. Single self-contained file, Tailwind via CDN, Fraunces + Geist via Google Fonts. Regenerate with:

```bash
pitchkit landing-page docs/ctx.json docs/index.html
git add docs && git commit -m "landing: refresh" && git push
```
